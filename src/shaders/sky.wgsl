...
fn calculate_sky_color(view_dir: vec3<f32>, sun_dir: vec3<f32>, moon_dir: vec3<f32>) -> vec3<f32> {
    ...
    let day_factor    = clamp(sun_height * 5.0, 0.0, 1.0);
    let night_factor  = clamp(-sun_height * 4.0 - 0.2, 0.0, 1.0);
    ...
    if sunset_factor > 0.01 && sun_height > -0.20 {
        ...
        let mie_sun = mie_phase(cos_theta, 0.76) / mie_phase(1.0, 0.76);

        let glow_core = pow(proximity_3d, 90.0);
        let glow_near = pow(proximity,    10.0);
        let glow_wide = pow(proximity,    3.5);
        ...
        sunset_color += vec3<f32>(1.00, 0.90, 0.50) * glow_core * 1.5;
        sunset_color += vec3<f32>(1.00, 0.45, 0.10) * glow_near * 0.9 * horizon_boost;
        sunset_color += vec3<f32>(0.85, 0.20, 0.08) * glow_wide * 0.35 * horizon_boost;
        sunset_color += vec3<f32>(1.00, 0.65, 0.25) * clamp(mie_sun, 0.0, 1.0) * 0.4 * sunset_intensity;
